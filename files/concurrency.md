...
[Синхронизаторы](files/lambda.md)  # todo  
...
### Метод yield  
С английского переводится как «уступать». И это ровно то, что метод делает!
Метод отдает поток выполнения другому потоку.
Пример:
```java
@Test  
public void test() {  
...
### Happens-Before  
**Happens-before** — это формальное гарантия, которую дает модель памяти Java (Java Memory Model, JMM) относительно порядка видимости операций с памятью, выполняемых в разных потоках.

#### 1. Правило монитора (блокировки)

- **Разблокировка монитора (`unlock`) happens-before последующая блокировка (`lock`) того же монитора.**
    
     Если поток A освободил замок (`unlock`) , и поток B захватил тот же замок (`lock`) , то поток B гарантированно увидит ВСЕ изменения, которые поток A сделал до освобождения замка (`unlock`) .
...
// Поток B
synchronized(lock) { // lock happens здесь -> гарантированно увидит counter = 42
    System.out.println(counter); // Выведет 42
}
```
    
#### 2. Правило `volatile` переменных

- **Запись в `volatile` переменную happens-before последующее чтение той же `volatile` переменной.**
    
    - Это одна из главных причин использовать `volatile`. Если поток A записал значение в `volatile`-поле, и поток B потом прочитал это поле, то поток B увидит не только новое значение поля, но и все остальные изменения памяти, сделанные потоком A до этой записи.
    Пример:
```java
...
if (flag) {          // (3) volatile чтение
    // Гарантированно увидит data = 100,
    // потому что (1) happens-before (2), а (2) happens-before (3)
    System.out.println(data); // Выведет 100
}
```

#### 3. Правило запуска и завершения потока

- **Вызов `thread.start()` happens-before любая операция внутри запущенного потока.**
    
    - Код, который инициализировал данные до `start()`, будет виден в новом потоке.
        
- **Любая операция в потоке happens-before другой поток завершит его вызовом `thread.join()` или определит его завершение через `thread.isAlive()`.**
    
    - Поток, вызвавший `join()`, гарантированно увидит все изменения, сделанные завершенным потоком.
        

#### 4. Правило инициализации (final поля)

- **Инициализация `final` полей в конструкторе happens-before любая операция с созданным объектом в другом потоке.**
    
    - Если объект правильно создан (на него не "убежала" ссылка из конструктора), то другие потоки гарантированно увидят проинициализированные `final`-поля, даже без синхронизации.

#### Итог

- **Happens-before** — это не временной порядок, а **гарантия видимости изменений** между потоками.
    
- Это отношение создается с помощью определенных языковых конструкций: `synchronized`, `volatile`, `Thread.start()`, `Thread.join()`, `final`-поля.
    
- Понимание happens-before **критически важно** для написания корректных и эффективных многопоточных приложений в Java, так как позволяет предсказать, когда один поток увидит изменения другого.
