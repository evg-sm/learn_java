...
После завершения фазы маркировки G1 знает, какие регионы в основном пусты. Сначала он собирается в этих областях, что обычно дает значительное количество свободного места (т. Е. Фаза 2, известная как «Подметание»). Вот почему этот метод сборки мусора называется Garbage-First.
