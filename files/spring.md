...
- Возможность интернационализации(поддержка разных языков).
- Возможность загрузки файлов.

### Может ли метод, помеченный аннотацией Bean, возвращать примитивный тип?  
Нет не может согласно документации Bean может отдавать только объекты.
Или уже может???
...
### Будет ли работать аннотация @Transactional с private методом?  
- С CGLIB работать не будет, private метод будет проигнорирован при создании прокси.
- Есть возможность сделать это с помощью AspectJ.
- При использовании стандартного механизма Spring Proxy AOP, то все функции AOP, предоставляемые Spring, включая @Transactional,
будут работать только через прокси. То есть при вызове из другого бина.  
**_Итого:_**
1. Если private метод вызван из того же бина, то он не будет учитываться (будет вызван не через прокси).
2. Если метод public, то он так же не будет учитываться, поскольку опять же вызов пойдет не через прокси.
...
Это шаблон проектирования, в котором в объект внедряются зависимости, а не создаются внутри объекта. Это отделяет создание объектов от их использования.
...
**_Содержит единственный метод:_**
...
- **_BeanFactory_** обеспечивает доступ к компонентам приложения
- **_ResourceLoader_** обеспечивает доступ к загрузке ресурсов (файлов)
- **_ApplicationEventPublisher_** возможность публикации событий
- **_MessageSource_** возможность интернационализации
- **_Автоматическая регистрация BeanPostProcessor и BeanFactoryPostProcessor_**  

### BeanPostProcessor 
Хук BeanFactory позволяет кастомно модифицировать bean'ы, до и после инициализации.

**_Содержит два метода:_**  
...
### @PostConstruct
**_Фактически init метод._**
Работает между методом BeanPostProcessor.**_postProcessBeforeInitialization_** и BeanPostProcessor.**_postProcessAfterInitialization_**.  

### Трехфазовый конструктор  
1. Java конструктор
2. @PostConstruct за который отвечает **_BeanPostProcessor_**
3. **_@AfterProxy_** после создания проксей спринга за него отвечает **_ContextListener_**  
...
Жизненный цикл контекста Spring состоит из 4-ёх этапов:

- **_Этап обновления (refresh) - автоматический_**
- **_Этап запуска (start) - вызывается методом ApplicationContext#start_**
- **_Этап остановки (stop) - вызывается методом ApplicationContext#stop_**
- **_Этап закрытия (close) - автоматический_**
...
2. ApplicationContext проверяет флаг SmartLifecycle#isRunning и вызывает метод SmartLifecycle#start, если флаг имеет значение false. _Да-да, контекст второй раз проходиться по объектам реализующие интерфейс SmartLifecycle_
