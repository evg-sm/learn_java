...
**_Работает только в рамках транзакции_** - если транзакции нет - блокировка установлена не будет.


## Индексы 

***Индекс в PostgreSQL*** — это отдельная структура данных, которая оптимизирует скорость поиска строк в таблице. Его работа основана на том же принципе, что и у книжного указателя: вместо того чтобы читать всю книгу подряд, вы находите нужное понятие в алфавитном списке и сразу переходите на нужную страницу.

Если кратко, то индекс хранит отсортированные значения одного или нескольких столбцов и ссылки (TID) на физические строки в таблице. Когда вы выполняете запрос с условием `WHERE`, PostgreSQL может быстро найти нужные значения в индексе и по ссылкам извлечь полные строки из таблицы
...
Можно создать индекс только на часть строк таблицы, используя условие `WHERE`. Это экономит место и ускоряет операции записи, если вы часто ищете по определенному сегменту данных (например, только по "активным" заказам)
### Функциональные индексы (**Индексы по выражениям**)

Можно индексировать не саму колонку, а результат функции или выражения. Например, индекс на `lower(email)` позволит быстро искать `WHERE lower(email) = 'user@example.com'`, не заботясь о регистре
...
Если в индекс добавить дополнительные колонки (неключевые), то для некоторых запросов PostgreSQL сможет вернуть результат, прочитав только индекс, вообще не обращаясь к таблице. Это называется **index-only scan**
